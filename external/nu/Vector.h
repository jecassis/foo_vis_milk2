...
class Vector
{
  ...
  public:
    ...
    Type* data() { return values; }
    Type* begin() const { return values; }
    Type* end() const
    {
        if (values)
            return values + numPtrs;
        else
            return 0;
    }
    void Reset()
    ...
    void clear() { numPtrs = 0; }
    size_t size() const { return numPtrs; }
    size_t capacity() { return allocSize; }
    Type& back() { return values[numPtrs - 1]; }

    Type& at(size_t index) const { return values[index]; }
    ...
    bool empty() const { return numPtrs == 0; }
